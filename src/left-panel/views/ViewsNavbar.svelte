<script>
  ...
  const getData = (tab) => { 
    ...
    let componentTree;
      ...
      arrSvelteFiles.map((resource) => {
        ...
        // bigData[JSON.parse(JSON.stringify(arrSvelteFiles[i]))] = 
        //   arrSvelteFiles[i].getContent(content => svelte.parse(content))
  ...
  setTimeout(() => {
      ...
      console.log('am i global', D3PreTree)
      let newD3Pre = []
      for (let eachObj of D3PreTree) {
        let temp = {}
        console.log('each obj in forloop', eachObj)
      ...
      console.log('D3 after mod', newD3Pre)
      ...
      for (let i = 0; i < AST.length; i++) {
        ...
        // for(let j = 0; j < AST[i].length; j++) {
        //   if (AST[i][j].type === 'ImportDeclaration') {
        //     if (AST[i][j].source.value !== 'svelte') {
        //       let obj = {}
        //       obj.type = AST[i][j].type
        //       obj.source = AST[i][j].source.value.split('')
        ...
        //   }  
        // }
        console.log('AST testing', AST)
      }
      console.log('ASTTTTTTTTTTt', AST)
      console.log('urls', urls)
      ...
      console.log('component Tempalte is', componentTemplate)
      ...
      console.log('compoenen template after add dataaa, ', componentTemplate)
      ...
      for (let i = 0; i < componentTemplate.length; i++) {
        if (!componentTemplate[i].hasOwnProperty('data')) {
          ...
          let result = []
          console.log('0', componentTemplate[i].data)
          console.log('1', Object.keys(componentTemplate[i].data.Props))
          componentTemplate[i].data.Props = result.concat(Object.keys(componentTemplate[i].data.Props))
        }
      }

      console.log('component tempalte final edit', componentTemplate)
  ...
  ///////start of tree
      ...
      root.x0 = height / 2;
      root.y0 = 0;
      console.log('root after hierarchy', root)

      // Collapse after the second level
      root.children.forEach(collapse);
      console.log('log after root.children')
      ...
      function update(source) {
        ...
        console.log('treeData inside updatae', treeData)
